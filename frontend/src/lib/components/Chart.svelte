<script lang="ts">
  ...
  import { initChart, ZOOM_STEP, zoomedWindow } from '$lib/echarts';
  ...
  let { option, height = '320px', onactivate, onitemclick }: Props = $props();
  ...
  // (see echarts.ts) because it steps far too hard; here each notch nudges the
