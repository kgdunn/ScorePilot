...
def apply_transform(
    ...
    *,
    ...
    - ``exponential``: ``exp(x)``
    - ``power``: ``sign(x) * abs(x) ** c1``. ``c1`` is treated as ``0.5`` when it
      is exactly ``0.0`` (the parameter's default), so the default is signed
      square root; any other value of ``c1`` (including negative exponents) is
