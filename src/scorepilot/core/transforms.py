...
def apply_transform(
    ...
    *,
    ...
    - ``exponential``: ``exp(x)``
    - ``power``: ``sign(x) * abs(x) ** (c1 if c1 != 0 else 0.5)`` — with the
      signature default ``c1 = 0.0`` this collapses to the signed square root;
      set ``c1`` to any non-zero value to use that as the exponent.
    """
