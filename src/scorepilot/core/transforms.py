...
def apply_transform(
    ...
    *,
    ...
    - ``exponential``: ``exp(x)``
    - ``power``: ``sign(x) * abs(x) ** c1``. ``c1 == 0.0`` (its parameter
      default) is treated as a sentinel for ``0.5`` (signed root), so the
      textbook default holds without needing to pass ``c1`` explicitly.
    """
