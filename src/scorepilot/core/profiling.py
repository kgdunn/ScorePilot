...
def suggest_transform(summary: VariableSummary) -> TransformKind:
    ...
    A strongly right-skewed (skewness > 1), strictly positive variable with a wide
    dynamic range (min-max ratio > 20) is a candidate for a log transform; a
    strongly-skewed variable that fails either the positivity or wide-range check
    gets a signed power (root). Otherwise no transform is suggested.
    """
