...
def suggest_transform(summary: VariableSummary) -> TransformKind:
    ...
    A strongly right-skewed (skewness > 1) variable is a candidate for
    transformation: when it is also strictly positive and has a wide dynamic
    range (min-max ratio > 20) a log transform is suggested; otherwise a signed
    power (root) is proposed as a milder alternative. A variable that is not
    strongly skewed gets no suggestion.
    """
